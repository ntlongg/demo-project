...
const CategoryList = () => {
  ...
  return (
    <div className='container mx-auto p-8 '>
        <div className='flex items-center gap-4 justify-between overflow-scroll scrollbar-none bottom-2 '>
            {
                    ...
                    categoryProduct.map((product)=>{
                        return(
                            <Link to={"/product-category/"+product?.category} className='cursor-pointer' key={product?.category}>
                                <div className='w-16 h-16 md:w-20 md:h-20 rounded-full  overflow-hidden p-4 bg-lime-400 flex items-center justify-center'>
                                    <img src={product?.productImage[0]} alt={product?.category} className=' h-full object-scale-down  hover:scale-125 transition-all'/>
                                </div>
                                <p className='text-center  hover:text-white text-sm md:text-base capitalize'>{product?.category}</p>
                            </Link>
                        )
                    })
                )
